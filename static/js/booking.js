...
function displayseat(){
    ...
    for(let i =1; i<=seatnum; i++)
    {
        ...
        if(seatarr.indexOf(i.toString())!== -1){
            rect.setAttribute("fill", "#9b0101");
            rect.setAttribute("id",i);
        }
        else{
            if(i<21){
                rect.setAttribute("fill", "#facde5");
                rect.setAttribute("color", "#ffffff");
                rect.addEventListener("click", handleClick);
                rect.setAttribute("id",i);
                rect.setAttribute("value",price[2]);
            }
            else if(i>20 && i<41){
                rect.setAttribute("fill", "#f18dbc");
                rect.addEventListener("click", handleClick);
                rect.setAttribute("id",i);
                rect.setAttribute("value",price[1]);
            }
            else{
                rect.setAttribute("fill", "#e0218a");
                rect.addEventListener("click", handleClick);
                rect.setAttribute("id",i);
...
function getdb(eventId) {
    fetch(`/auth/api/events/${eventId}`)
        ...
        .then(eventDetails => {
            ...
            document.getElementById('movie_Name').textContent = `Movie Name: ${eventDetails.eventname} `;
            document.getElementById('movie_Date').textContent = `Date: ${eventDetails.date} `;
            document.getElementById('movie_Time').textContent = `Time: ${eventDetails.time}`;
            document.getElementById('movie_Venue').textContent = `Venue: ${eventDetails.venue}`;
            document.getElementById('movie_description').textContent = `Description: ${eventDetails.description}`;
            displayseat();
        })
        .catch(error => console.error('Error fetching event details:', error));
}
function generateReceipt(movieDetails) {
